...
<Callout type="info">
  **Architectural Note:** shadcn/ui components (Button, Card, Input, etc.) are **not bundled** in `@bettercone/ui`. The CLI installs them separately in your `components/ui/` folder for optimal bundle size and customization.
</Callout>
